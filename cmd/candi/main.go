...
func main() {
	...
	var data param
	data.Header = fmt.Sprintf("Code generated by candi %s.", version)
	...
	dataSourceWithHandler := map[string]string{"PackageName": packageName, "ServiceName": data.ServiceName, "Header": data.Header}
	...
	for _, mod := range data.Modules {
		...
		domainFiles = append(domainFiles, FileStructure{
			Source: "package domain", Skip: mod.Skip, FileName: mod.Name + ".go",
		})
	}
		...
		baseDirectoryFile.Childs = []FileStructure{
			...
			{TargetDir: "pkg/", IsDir: true, Childs: []FileStructure{
				...
				{TargetDir: "shared/", IsDir: true, Childs: []FileStructure{
					{TargetDir: "domain/", IsDir: true, Childs: domainFiles},
					{TargetDir: "repository/", IsDir: true, Childs: []FileStructure{
						{FromTemplate: true, DataSource: data, Source: templateRepositorySQL, FileName: "repository_sql.go"},
					}},
				}},
			}},
	...
	case addModule:
		...
		baseDirectoryFile.Childs = []FileStructure{
			...
			{TargetDir: "pkg/", IsDir: true, Skip: true, Childs: []FileStructure{
				{TargetDir: "shared/", IsDir: true, Skip: true, Childs: []FileStructure{
					{TargetDir: "domain/", IsDir: true, Skip: true, Childs: domainFiles},
				}},
			}},
		}
