...
// IsLockedTTLWithLimit provides a mock function with given fields: key, limit, TTL
func (_m *Locker) IsLockedTTLWithLimit(key string, limit int, TTL time.Duration) bool {
	ret := _m.Called(key, limit, TTL)

	if len(ret) == 0 {
		panic("no return value specified for IsLockedTTLWithLimit")
	}

	var r0 bool
	if rf, ok := ret.Get(0).(func(string, int, time.Duration) bool); ok {
		r0 = rf(key, limit, TTL)
	} else {
		r0 = ret.Get(0).(bool)
	}
